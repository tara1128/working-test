...
datas: {
    ...
    "pmp-054": {
      ...
      tools: [
        {text: '分解（自上而下）Decomposition', cusWidth: 620, ext: [
          {text: '项目下分为“阶段”，整个项目为第一层，“阶段”为第二层，再往下分，每个阶段分出多个可交付成果'},
          {text: '工作包Work Packages是WBS的最底层，工作包不是工作，而是一个可交付成果（例如装修房间，一个灯泡是一个可交付成果，而安装调试灯泡，才是工作；可以把“灯泡”这个可交付成果，向下分解为“安装灯泡”和“调试灯泡”这两个活动），可以对其进行成本估算和管理（第七章）、持续时间估算和管理（第六章）'},
    ...
    "pmp-056": {
      ...
      processGroup: '监控过程组 <b>Monitoring and Controlling Process Group</b>',
      keyNotes: [
        {text: '监督项目和产品的范围状态，管理范围基准变更'},
        {text: '在整个项目期间，保持对范围基准的维护'},
        {text: '确保所有请求的变更、推荐的纠正措施或预防措施，都经过实施整体变更控制过程的处理'},
        {text: '控制范围，即管理变更，不是批准变更请求，而是提交变更请求，并且要到整体变更控制过程去批准或否决'},
        {text: '范围蔓延Scope Creep（范围潜变）：未经控制的产品或项目范围的扩大，未对时间、成本、资源做相应调整'},
        {text: '镀金Gold-plating：在范围定义之外，团队主动增加的额外工作。镀金的项目是失败的，浪费了资源！'},
        {text: 'PMI主义（PMI-Isms）：拒绝镀金，拒绝额外工作（Extra）'},
        {text: '蔓延：被动的范围扩大；镀金：主动的范围扩大'}
      ],
      inputs: [
        {text: '项目管理计划'},
        {text: '需求文件', ext: [{text: '5.2 收集需求的输出'}]},
      ...
      tools: [
        {text: '偏差分析', cusWidth: 420, ext: [
          {text: 'Variance Analysis，绩效与计划做对比，确定实际绩效与基准的差异程度和原因，决定是否采取纠正措施或预防措施'},
          {text: '可利用项目绩效测量结果，来评估偏离范围基准的程度'}
        ]}
      ],
      outputs: [
        {text: '工作绩效信息', cusWidth: 420, ext: [
          {text: '有关项目范围实施情况（对照范围基准）、相互关联且与背景相结合的信息'},
          {text: '注：绩效数据是测量的原始数据、绩效信息是经过分析后得出的信息'}
        ]},
        {text: '变更请求'},
        {text: '项目管理计划更新', cusWidth: 420, ext: [
          {text: '范围基准（更新）'},
          {text: '其他基准（更新）：项目范围变了，相应的成本基准、进度基准等都要重新修订和发布'}
        ]},
        {text: '项目文件更新'},
        {text: '组织过程资产更新'}
      ],
